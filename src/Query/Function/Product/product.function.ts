...
//Get Setup times
export const GET_SET_UP_TIMES = async (date: string, id: string): Promise<OrderDetailsForTiming[]> => await (await fetch(`https://wdg.teamdesk.net/secure/api/v2/90582/Order/select.json?filter=ToDate(%5BRental%20Date%5D)%20%3D%20ToDate(%22${date}%22)%20and%20%5BReference%20to%20Admin%20-%20User%20Property%5D%3DToUser(%22${getOwner(id)}%22)`, { headers }).then(res => res.json()));


//Get Removal times
export const GET_REMOVAL_TIMES = async (date: string, id: string): Promise<OrderDetailsForTiming[]> => await (await fetch(`https://wdg.teamdesk.net/secure/api/v2/90582/Order/select.json?filter=ToDate(%5BReturn%20Date%5D)%20%3D%20ToDate(%22${date}%22)%20and%20%5BReference%20to%20Admin%20-%20User%20Property%5D%3DToUser(%22${getOwner(id)}%22)`, { headers }).then(res => res.json()));
